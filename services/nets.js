...
exports.generateQrCode = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      // Initialize txn status as pending in app locals store for SSE
      try {
        if (req.app && req.app.locals && req.app.locals.netsTxnStatus) {
          req.app.locals.netsTxnStatus.set(txnRetrievalRef, { status: 'pending' });
        }
      } catch (e) {
        console.warn('Could not init txn status store:', e && e.message ? e.message : e);
      }
